// Crank Puzzle Controller — v8 (Teensy 4.1)
//
...
static sentient_v8::Client sne_encoder_a(make_cfg("crank_encoder_a", HMAC_ENCODER_A));
static sentient_v8::Client sne_encoder_b(make_cfg("crank_encoder_b", HMAC_ENCODER_B));

enum class DeviceKind : uint8_t { EncoderA, EncoderB };
struct DeviceCtx {
  DeviceKind kind;
};
...
static void publish_state(long a, long b) {
  {
...
static bool handleCommand(const JsonDocument &cmd, JsonDocument &rejectedAckReason, void *vctx) {
  ...
  // Encoders are input-only, but allow simple ops for commissioning.
